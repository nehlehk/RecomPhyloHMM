...
params.out =  "${PWD}/results_philobacteria"
...
process phyloHMM_two {
     ...
     """
       python3.8  $PWD/bin/phyloHMM_2states.py -t ${myRaxML} -a ${wholegenome}  -x ${params.xml_file} -l ${recomlog}  -c ${CFML_recom} -nu ${nu_hmm} -p ${mix_prob}
...
process RMSE_summary {
     ...
     publishDir "$PWD/results_philobacteria/Summary_Results", mode: "copy"
     maxForks 1
     label 'short'     
...
process RMSE_summary_states {
     ...
     """
       python3.8  $PWD/bin/rmse_plot_states.py -t rmse_phylohmm_two.csv  -f rmse_phylohmm_four.csv -c rmse_CFML.csv
...
process treeannotator_partial_four {
     ...
     """
       treeannotator -b 10  ${beastPartialTree_four}  beastOurTree_four.nexus      
     """
}
...
process convertor_ourTree_four {
     ...
     """
       python3.8  $PWD/bin/NexusToNewick.py -t ${beastOurTree_four} -o 'beastHMMTree_four.newick'
...
process treeannotator_partial_two {
     ...
     """
       treeannotator -b 10  ${beastPartialTree_two}  beastOurTree_two.nexus      
     """
}
...
process convertor_ourTree_two {
     ...
     """
       python3.8  $PWD/bin/NexusToNewick.py -t ${beastOurTree_two} -o 'beastHMMTree_two.newick'
...
process mergeTreeFiles {
    ...
    output:
     ...
     """
       python3.8  $PWD/bin/mergeFiles.py ${beastHMMTree_four}  ${beastHMMTree_two}  ${myRaxML}  ${beastTree}  ${CFMLtree}  ${gubbinstree} > allOtherTrees.newick
...
process TreeCmp_summary {
     ...
     """
       python3.8 $PWD/bin/cmpTree_plot.py -c all_cmpTrees.result
...
workflow {
    ...
    collectedRMSE_HMM_two = phyloHMM_two.out.rmse_phylohmm_two.collectFile(name:"rmse_phylohmm_two.csv",storeDir:"${PWD}/results_philobacteria/Summary_Results", keepHeader:false , sort: false) 
    
    collectedRMSE_HMM_four = phyloHMM_four.out.rmse_phylohmm_four.collectFile(name:"rmse_phylohmm_four.csv",storeDir:"${PWD}/results_philobacteria/Summary_Results", keepHeader:false , sort: false)
    
    collectedRMSE_CFML = CFML_result.out.rmse_CFML.collectFile(name:"rmse_CFML.csv",storeDir:"${PWD}/results_philobacteria/Summary_Results", keepHeader:false , sort: false) 
...
//    RMSE_summary(collectedRMSE_HMM_four,collectedRMSE_CFML)
    ...
    collectedCMP_tree = TreeCmp.out.Comparison.collectFile(name:"all_cmpTrees.result",storeDir:"${PWD}/results_philobacteria/Summary_Results", keepHeader:false , sort: false) 
